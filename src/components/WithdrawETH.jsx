...
const WithdrawETH = () => {
  ...
  const ethBalance = assetBalance(presentUser, "ethereum");
  const currentDate = new Date();
  const formattedDate = formatDate(currentDate);
  const formattedTime = formatTime(currentDate);
  const { response } = useAxios(`coins/ethereum/`);

  const ethBalanceInUSD =
    +ethBalance * +response?.market_data?.current_price?.usd;
  ...
  const validateForm = () => {
    ...
    } else if (currencyType === "ETH" && quantity > ethBalance) {
      newFormErrors = {
        ...newFormErrors,
        quantity: "You have exceeded your available balance",
      };
    } else if (currencyType === "USD" && quantity > ethBalanceInUSD) {
      newFormErrors = {
        ...newFormErrors,
        quantity: "You have exceeded your available balance",
      };
  ...
  const max = () => {
    ...
    } else setQuantity(ethBalanceInUSD);
  };
      ...
      <div className="p-4 font-poppins">
        ...
        <div className="mt-4">
          ...
          <p className="my-1">
            ...
            <span className="font-semibold text-white text-[14px]">
              {currencyType === "ETH" && `${ethBalance?.toFixed(4)} ETH`}
              {currencyType === "USD" && `$${ethBalanceInUSD?.toFixed(2)}`}
            </span>
          </p>
        </div>
        <div className="my-10 border-t-2 border-slate-600">
          ...
          <div className="flex justify-between items-center my-3">
            ...
            <p className="text-[14px]">less than a minute</p>
          </div>
          <div className="flex justify-between items-center my-3">
